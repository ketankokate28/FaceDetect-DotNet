...
namespace FaceRecognition
{
    class Program
    {
        static void Main()
        {
            Console.WriteLine("FaceONNX: Multi-Suspect Augmented Face Matching");
            ...
            foreach (var suspectFile in suspectFiles)
            {
                ...
                if (faces.Length == 0)
                {
                    Console.WriteLine($"No face found in suspect image: {suspectFile}");
                    continue;
                }
                ...
                Console.WriteLine($"Loaded suspect: {name} with {embeddings.Count} augmented embeddings");
            }
            ...
            foreach (var imageFile in groupImages)
            {
                    ...
                    foreach (var face in groupFaces)
                    {
                        ...
                        foreach (var (name, embedding) in suspectEmbeddings)
                        {
                            ...
                            if (distance < bestDistance && distance < 0.8)
                            {
                                ...
                                Console.WriteLine($"Loaded suspect: {matchedName} with distance {bestDistance}");
                ...
                try
                {
                    ...
                    Console.WriteLine($"Deleted input image: {Path.GetFileName(imageFile)}");
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"Failed to delete image: {imageFile}. Error: {ex.Message}");
                }
            }


            Console.WriteLine("Face matching complete.");
            Console.ReadKey();
        }
